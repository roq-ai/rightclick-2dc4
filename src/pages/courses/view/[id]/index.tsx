            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Course Creator Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_course_creator_idTouser?.id}`}>
                        {data?.user_course_creator_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Course Instructor Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_course_instructor_idTouser?.id}`}>
                        {data?.user_course_instructor_idTouser?.email}
                      </Link>
                    }
                  />
                )}
