...
export function CourseListPage(props: CourseListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCourses({
        relations: ['user_course_creator_idTouser', 'user_course_instructor_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_course_creator_idTouser',
          header: 'User Course Creator Id Touser',
          accessorKey: 'user_course_creator_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_course_creator_idTouser?.id}`}
            >
              {record.user_course_creator_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_course_instructor_idTouser',
          header: 'User Course Instructor Id Touser',
          accessorKey: 'user_course_instructor_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_course_instructor_idTouser?.id}`}
            >
              {record.user_course_instructor_idTouser?.email}
            </Link>
          ),
        }
      : null,
